...
fn validate_clone_inputs(repo_url: &str, base_branch: &str, doc_path: &str) -> Result<(), DocError> {
    if repo_url.starts_with('-') || base_branch.starts_with('-') {
        return Err(DocError::Failed("repo_url/base_branch must not start with '-'".into()));
    }
...
impl DocSource for GitDocSource {
    async fn read(&self, repo_url: &str, base_branch: &str, doc_path: &str) -> Result<String, DocError> {
        ...
        let base = tokio::fs::canonicalize(&dir).await
            .map_err(|e| DocError::Failed(e.to_string()))?;
        let target = tokio::fs::canonicalize(base.join(doc_path)).await
