...
fn poll_until_ok(cfg: &ProbeCfg, probe: &dyn Probe, clock: &dyn Clock) -> bool {
    ...
    loop {
        if let Some(code) = probe.probe(&cfg.url) {
            if cfg.ok_status.contains(&code) {
                return true;
            }
        }
        if clock.now_ms().saturating_sub(start) >= cfg.timeout {
            return false;
        }
...
fn both_probes_pass(m: &Manifest, probe: &dyn Probe) -> bool {
    let h = probe
        .probe(&m.lifecycle.health.url)
        .map(|c| m.lifecycle.health.ok_status.contains(&c))
        .unwrap_or(false);
    if !h {
        return false;
    }
    probe
        .probe(&m.lifecycle.ready.url)
        .map(|c| m.lifecycle.ready.ok_status.contains(&c))
        .unwrap_or(false)
}
...
pub fn run_start_sequence(
    ...
    if let Some(build) = &m.lifecycle.build {
        if !images_present {
            ...
            if code != 0 {
                sink.emit_state(&m.id, ERROR);
                return StartOutcome::Error(format!("build exited {code}"));
            }
        }
    }
    ...
    if !poll_until_ok(&m.lifecycle.health, probe, clock) {
        sink.emit_state(&m.id, ERROR);
        return StartOutcome::Error("health probe timed out".into());
    }
    sink.emit_state(&m.id, READY_PROBING);
    if !poll_until_ok(&m.lifecycle.ready, probe, clock) {
        sink.emit_state(&m.id, ERROR);
        return StartOutcome::Error("ready probe timed out".into());
    }
...
mod tests {
    ...
    fn manifest() -> Manifest {
        Manifest::from_json(
            r#"{"id":"audience","name":"Audience","apiVersion":1,
          ...
          "lifecycle":{"cwd":"/x","build":{"cmd":"build","timeout":1000},
            ...
            "ready":{"url":"r","okStatus":[200,302],"timeout":5000,"interval":1000}}}"#,
        )
        .unwrap()
    }
